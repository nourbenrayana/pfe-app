...
class AbstractDocumentQuery extends node_events_1.EventEmitter {
    ...
    _buildInclude(queryText) {
        ...
        if (this._queryTimings) {
            ...
            firstRef.value = false;
            TimingsToken_js_1.TimingsToken.instance.writeTo(queryText);
        }
    }
    ...
    _intersect() {
        ...
        if (tokens.length > 0) {
            ...
            if (last instanceof WhereToken_js_1.WhereToken || last instanceof CloseSubclauseToken_js_1.CloseSubclauseToken) {
                this._isIntersect = true;
                tokens.push(IntersectMarkerToken_js_1.IntersectMarkerToken.INSTANCE);
                return;
            }
        }
        (0, index_js_1.throwError)("InvalidOperationException", "Cannot add INTERSECT at this point.");
    }
    _whereExists(fieldName) {
        ...
        this._appendOperatorIfNeeded(tokens);
        this._negateIfNeeded(tokens, null);
        tokens.push(WhereToken_js_1.WhereToken.create("Exists", fieldName, null));
    }
    _containsAny(fieldName, values) {
        ...
        const whereToken = WhereToken_js_1.WhereToken.create("In", fieldName, this._addQueryParameter(array), new WhereToken_js_1.WhereOptions({ exact: false }));
        tokens.push(whereToken);
    }
    _containsAll(fieldName, values) {
        ...
        const whereToken = WhereToken_js_1.WhereToken.create("AllIn", fieldName, this._addQueryParameter(array));
        tokens.push(whereToken);
    }
    addRootType(clazz) {
        this._rootTypes.add(clazz);
    ...
    _distinct() {
        if (this.isDistinct) {
            (0, index_js_1.throwError)("InvalidOperationException", "This is already a distinct query.");
        }
        if (!this._selectTokens.length) {
            this._selectTokens.push(DistinctToken_js_1.DistinctToken.INSTANCE);
        }
        else {
            this._selectTokens.unshift(DistinctToken_js_1.DistinctToken.INSTANCE);
        }
    }
    ...
    _buildSelect(writer) {
        ...
        if (this._selectTokens.length === 1 && this._selectTokens[0] instanceof DistinctToken_js_1.DistinctToken) {
        ...
        for (let i = 0; i < this._selectTokens.length; i++) {
            ...
            if (i > 0 && !(this._selectTokens[i - 1] instanceof DistinctToken_js_1.DistinctToken)) {
                writer.append(",");
            }
            DocumentQueryHelper_js_1.DocumentQueryHelper.addSpaceIfNeeded(i > 0 ? this._selectTokens[i - 1] : null, token, writer);
    ...
    _buildWhere(writer) {
        ...
        for (let i = 0; i < this._whereTokens.length; i++) {
            DocumentQueryHelper_js_1.DocumentQueryHelper.addSpaceIfNeeded(i > 0 ? this._whereTokens[i - 1] : null, this._whereTokens[i], writer);
    ...
    _buildFilter(writer) {
        ...
        for (let i = 0; i < this._filterTokens.length; i++) {
            DocumentQueryHelper_js_1.DocumentQueryHelper.addSpaceIfNeeded(i > 0 ? this._filterTokens[i - 1] : null, this._filterTokens[i], writer);
    ...
    _includeTimings(timingsCallback) {
        ...
        this._queryTimings = new QueryTimings_js_1.QueryTimings();
        timingsCallback(this._queryTimings);
    }
    _highlight(parameters, highlightingsCallback) {
        ...
        const optionsParameterName = parameters
            ? this._addQueryParameter((0, HighlightingOptions_js_1.extractHighlightingOptionsFromParameters)(parameters))
            : null;
        const token = HighlightingToken_js_1.HighlightingToken.create(parameters.fieldName, parameters.fragmentLength, parameters.fragmentCount, optionsParameterName);
    ...
    _withinRadiusOf(fieldName, radius, latitude, longitude, radiusUnits, distErrorPercent) {
        ...
        this._appendOperatorIfNeeded(tokens);
        this._negateIfNeeded(tokens, fieldName);
        const whereToken = WhereToken_js_1.WhereToken.create("SpatialWithin", fieldName, null, new WhereToken_js_1.WhereOptions({
            shape: ShapeToken_js_1.ShapeToken.circle(this._addQueryParameter(radius), this._addQueryParameter(latitude), this._addQueryParameter(longitude), radiusUnits),
    ...
    _spatialByShapeWkt(fieldName, shapeWkt, relation, units, distErrorPercent) {
        ...
        this._appendOperatorIfNeeded(tokens);
        this._negateIfNeeded(tokens, fieldName);
        const wktToken = ShapeToken_js_1.ShapeToken.wkt(this._addQueryParameter(shapeWkt), units);
        let whereOperator;
        switch (relation) {
            case "Within": {
            ...
            default: {
                (0, index_js_1.throwError)("InvalidArgumentException", `relation: ${relation}.`);
            }
        }
        tokens.push(WhereToken_js_1.WhereToken.create(whereOperator, fieldName, null, new WhereToken_js_1.WhereOptions({
            shape: wktToken,
            distance: distErrorPercent
        })));
    }
    ...
    _orderByDistance(fieldNameOrField, shapeWktOrLatitude, longitudeOrRoundFactor, roundFactor) {
        if (TypeUtil_js_1.TypeUtil.isString(fieldNameOrField)) {
            if (TypeUtil_js_1.TypeUtil.isString(shapeWktOrLatitude)) {
                const roundFactorParameterName = longitudeOrRoundFactor ? this._addQueryParameter(longitudeOrRoundFactor) : null;
                this._orderByTokens.push(OrderByToken_js_1.OrderByToken.createDistanceAscending(fieldNameOrField, this._addQueryParameter(shapeWktOrLatitude), roundFactorParameterName));
            }
            else {
                const roundFactorParameterName = roundFactor ? this._addQueryParameter(roundFactor) : null;
                this._orderByTokens.push(OrderByToken_js_1.OrderByToken.createDistanceAscending(fieldNameOrField, this._addQueryParameter(shapeWktOrLatitude), this._addQueryParameter(longitudeOrRoundFactor), roundFactorParameterName));
            }
            return;
        }
    ...
    _orderByDistanceDescending(fieldNameOrField, shapeWktOrLatitude, longitudeOrRoundFactor, roundFactor) {
        if (TypeUtil_js_1.TypeUtil.isString(fieldNameOrField)) {
            if (TypeUtil_js_1.TypeUtil.isString(shapeWktOrLatitude)) {
                const roundFactorParameterName = longitudeOrRoundFactor ? this._addQueryParameter(longitudeOrRoundFactor) : null;
                this._orderByTokens.push(OrderByToken_js_1.OrderByToken.createDistanceDescending(fieldNameOrField, this._addQueryParameter(shapeWktOrLatitude), roundFactorParameterName));
            }
            else {
                const roundFactorParameterName = roundFactor ? this._addQueryParameter(roundFactor) : null;
                this._orderByTokens.push(OrderByToken_js_1.OrderByToken.createDistanceDescending(fieldNameOrField, this._addQueryParameter(shapeWktOrLatitude), this._addQueryParameter(longitudeOrRoundFactor), roundFactorParameterName));
            }
            return;
        }
        const field = fieldNameOrField;
    ...
    _assertIsDynamicQuery(dynamicField, methodName) {
        if (!this._fromToken.isDynamic) {
            (0, index_js_1.throwError)("InvalidOperationException", "Cannot execute query method '" + methodName +
    ...
    async first() {
        ...
        if (entries.length === 0) {
            (0, index_js_1.throwError)("InvalidOperationException", "Expected at least one result.");
        }
        return entries[0];
    }
    async firstOrNull() {
    ...
    async single() {
        ...
        if (entries.length !== 1) {
            (0, index_js_1.throwError)("InvalidOperationException", `Expected single result, but got ${entries.length ? "more than that" : 0}.`);
        }
        return entries[0];
    }
    async singleOrNull() {
        const entries = await this._executeQueryOperation(2);
        if (entries.length === 2) {
            (0, index_js_1.throwError)("InvalidOperationException", `Expected single result, but got more than that.`);
        }
    ...
    _aggregateBy(facet) {
        for (const token of this._selectTokens) {
            if (token instanceof FacetToken_js_1.FacetToken) {
                continue;
            }
            (0, index_js_1.throwError)("InvalidOperationException", "Aggregation query can select only facets while it got " + token.constructor.name + " token");
        }
        const facetToken = FacetToken_js_1.FacetToken.create(facet, (val) => this._addQueryParameter(val));
        this._selectTokens.push(facetToken);
    }
    _aggregateUsing(facetSetupDocumentId) {
        this._selectTokens.push(FacetToken_js_1.FacetToken.create(facetSetupDocumentId));
    }
    lazily() {
    ...
    countLazily() {
        ...
        const clazz = this._conventions.getJsTypeByDocumentType(this._clazz);
        const lazyQueryOperation = new LazyQueryOperation_js_1.LazyQueryOperation(this._theSession, this._queryOperation, this, clazz);
        return this._theSession.addLazyCountOperation(lazyQueryOperation);
    }
    _suggestUsing(suggestion) {
        if (!suggestion) {
            (0, index_js_1.throwError)("InvalidArgumentException", "suggestion cannot be null");
        }
        this._assertCanSuggest(suggestion);
        let token = null;
        if (suggestion instanceof SuggestionWithTerm_js_1.SuggestionWithTerm) {
            const term = suggestion;
            token = SuggestToken_js_1.SuggestToken.create(term.field, term.displayField, this._addQueryParameter(term.term), this._getOptionsParameterName(term.options));
        }
        else if (suggestion instanceof SuggestionWithTerms_js_1.SuggestionWithTerms) {
            const terms = suggestion;
            token = SuggestToken_js_1.SuggestToken.create(terms.field, terms.displayField, this._addQueryParameter(terms.terms), this._getOptionsParameterName(terms.options));
        }
        else {
            (0, index_js_1.throwError)("InvalidOperationException", "Unknown type of suggestion: " + suggestion);
        }
    ...
    _assertCanSuggest(suggestion) {
        if (this._whereTokens.length) {
            (0, index_js_1.throwError)("InvalidOperationException", "Cannot add suggest when WHERE statements are present.");
        }
        if (this._selectTokens.length) {
            const lastToken = this._selectTokens.at(-1);
            if (lastToken instanceof SuggestToken_js_1.SuggestToken) {
                if (lastToken.fieldName === suggestion.field) {
                    (0, index_js_1.throwError)("InvalidOperationException", "Cannot add suggest for the same field again.");
                }
            }
            else {
                (0, index_js_1.throwError)("InvalidOperationException", "Cannot add suggest when SELECT statements are present.");
            }
        }
        if (this._orderByTokens.length) {
            (0, index_js_1.throwError)("InvalidOperationException", "Cannot add suggest when ORDER BY statements are present.");
        }
    }
    _includeExplanations(options, explanationsCallback) {
        if (this._explanationToken) {
            (0, index_js_1.throwError)("InvalidOperationException", "Duplicate IncludeExplanations method calls are forbidden.");
        }
        const optionsParameterName = options
            ? this._addQueryParameter(options)
            : null;
        this._explanationToken = ExplanationToken_js_1.ExplanationToken.create(optionsParameterName);
        this._explanations = new Explanations_js_1.Explanations();
    ...
    _includeCounters(alias, counterToIncludeByDocId) {
        ...
        for (const [key, val] of counterToIncludeByDocId.entries()) {
            if (val[0]) {
                this._counterIncludesTokens.push(CounterIncludesToken_js_1.CounterIncludesToken.all(key));
                continue;
            }
            const valArr = [...val[1]];
            ...
            for (const name of val[1]) {
                this._counterIncludesTokens.push(CounterIncludesToken_js_1.CounterIncludesToken.create(key, name));
            }
        }
    }
    _includeTimeSeries(alias, timeSeriesToInclude) {
        ...
        for (const kvp of timeSeriesToInclude.entries()) {
            for (const range of kvp[1].values()) {
                this._timeSeriesIncludesTokens.push(TimeSeriesIncludesToken_js_1.TimeSeriesIncludesToken.create(kvp[0], range));
            }
        }
    }
    getQueryType() {
    ...
    _addFromAliasToTokens(fromAlias, tokens) {
        if (!fromAlias) {
            (0, index_js_1.throwError)("InvalidArgumentException", "Alias cannot be null or empty.");
        }
        for (const token of tokens) {
            if (token instanceof WhereToken_js_1.WhereToken) {
                token.addAlias(fromAlias);
            }
            if (token instanceof OrderByToken_js_1.OrderByToken) {
                token.addAlias(fromAlias);
            }
        }
    }
    ...
    _includeRevisionsByDate(dateTime) {
        ...
        this._revisionsIncludesTokens.push(RevisionIncludesToken_js_1.RevisionIncludesToken.createForDate(dateTime));
    }
    _includeRevisionsByChangeVector(revisionsToIncludeByChangeVector) {
        ...
        for (const changeVector of revisionsToIncludeByChangeVector) {
            this._revisionsIncludesTokens.push(RevisionIncludesToken_js_1.RevisionIncludesToken.createForPath(changeVector));
        }
    }
    get parameterPrefix() {
